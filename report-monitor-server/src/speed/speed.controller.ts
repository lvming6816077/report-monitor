...
export class SpeedController {
    ...
    @Get('getTagsList')
    async getTagsList(@Query() query: QuerySpeedTagDto): Promise<PaginateResult<SpeedTagDocument>> {
        return this.speedService.findAllTagByPage(query.pageStart, query.pageSize, query);
    }
    @Get('getTags')
    ...
    async getPointsList(@Query() query: QuerySpeedDto) {
        let result = await this.speedService.findAllByPage(query.pageStart, query.pageSize, query);
    ...
    async getSpeedsAndSpeedset(@Request() req: any): Promise<any> {
        let speedset = await this.userService.findUserSpeedSet(req.user.userId);
        let projectId = req.query.projectId

        let list = await this.speedService.findAll(projectId);
        return {
            speedset:speedset,
            list
        }
