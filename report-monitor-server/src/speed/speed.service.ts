...
export class SpeedService {
    constructor(
        ...
        private readonly speedModel: PaginateModel<SpeedDocument>,

        @InjectModel(SpeedTag.name)
        private readonly speedTagModel: PaginateModel<SpeedTagDocument>,
    ...
    async createTag(desc: string, projectId: string): Promise<SpeedTag> {
        ...
        return await this.speedTagModel.create({
            desc: desc,
    ...
    async findAllTags(query: QuerySpeedTagDto): Promise<SpeedTag[]> {
        const list = await this.speedTagModel
    ...
    async findAllByPage(
        ...
        query: QuerySpeedDto,
        isAll = false,
    ): Promise<PaginateResult<SpeedDocument>> {
        const options = {
            populate: ['tag'],
            page: Number(pageStart),
            limit: Number(pageSize),
    ...
    async findAll(projectId: string): Promise<any[]> {
        ...
        for (let i = 0; i < list.length; i++) {
            const children = this.speedModel.find({ tag: list[i]._id }).exec();
            const o = {
                title: list[i].desc,
                key: list[i]._id,
                value: list[i]._id,
