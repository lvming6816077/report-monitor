...
export class UserController {
    ...
    async login(@Body() body: LoginDto, @Req() req) {
        ...
        const u: User = await this.userService.findUser(
            body.username,
            body.password,
        );
        if (u) {
    ...
    async getUsersList(@Query() query: QueryUserDto) {
        const result = await this.userService.findAllByPage(
            query.pageStart,
            query.pageSize,
            query,
        );
    ...
    async sendEmailCode(@Query() query, @Request() req: any) {
        ...
        const { username } = await this.userService.findUserByUserId(
            req.user.userId,
        );
        ...
        //随机获取6位数字
        const s2msCode = Math.random().toString().slice(-6);
