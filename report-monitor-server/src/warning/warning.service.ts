...
export class WarningService {
    ...
    async findAllOpened(): Promise<WarningDocument[]> {
        const res = await this.warningModel
            .find({
                isOpen: true,
            })
            .populate({
    ...
    async updateWarningTrigger(
        id: string,
        num?: number,
    ): Promise<WarningDocument> {
        // const r = await this.warningModel.findById(id).exec()

        return await this.warningModel.findOneAndUpdate(
            { _id: id },
            {
    ...
    async dealSingleTask(item: WarningDocument) {
        ...
        const result = await this.reportService.findAllByCode(
            code,
            cur,
            next,
            interval,
        ...
        for (let i = 0; i < result.list.length; i++) {
            const o = result.list[i] as any;
            const { lastTriggerDate } = await this.findById(item._id);

            if (
                o.count >= max &&
                (!lastTriggerDate ||
                    moment(lastTriggerDate)
                        .add(interval, 'minute')
                        .isBefore(moment()))
            ) {
                // 在时间间隔内只出发一次
    ...
    async trigger(item: WarningDocument) {
        ...
        const res = formatData(data.list, start, end, 5);
        ...
        const u = await this.userModel
            .findOne({ userid: item.point.user as unknown as string })
            .exec();
